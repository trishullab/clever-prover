...
Start the code by writing [START CODE] and end the code by writing [END CODE].""" # TODO: maybe find a better way to handle presence/absence of plan
...
Start the code by writing [START CODE] and end the code by writing [END CODE].""" # TODO: maybe adjust [START] and [END] scaffolding (does "```python" work better?)
    ...
    def parse_response(self, response: str) -> str:
        actual_code_ind = response.find("[START CODE]")
        if actual_code_ind != -1:
            response = response[(actual_code_ind + len("[START CODE]")):]
