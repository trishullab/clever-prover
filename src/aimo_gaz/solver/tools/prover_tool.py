...
class ProverTool(Tool): # TODO: ignoring all actions other than RUN_TACTIC for now
    ...
    def solve_intermediate(self, problem_description: str, proof_env: ProofEnv, solution_str: str, tool_prompt: str) -> typing.Tuple[str, float]: # TODO: maybe make solution_str part of proof_env instead of passing it separately
        if not self.model.is_loaded():
            self.model.__enter__()
        # Prompt the model for the tactic
        self.history = self.prompter.get_prompt(self.history, problem_description, proof_env, solution_str, tool_prompt)
