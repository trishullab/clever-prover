...
from clever_prover.tasks.implementation_generation_task import ImplementationGenerationTask, GenerationResult
...
from clever_prover.solver.tools.few_shot_impl_prover_tool import FewShotImplProverTool
...
class ImplGenerator(ImplementationGenerationTask):
    """
    ...
    def generate_implementation_correctness_proof(self, timeout_in_ms = 60, logger = None):
        ...
        while not is_time_elapsed and not proof_found and proof_sample_count < self.num_proof_plan_samples:
            ...
            if plan_generated or not self.use_proof_planner:
                # Remove lemma_plans which are already proven
                ...
                if len(proofs_found) == len(lemma_plans):
                    logger.info("All lemmas proven, generating proof for correctness theorem.")
                ...
                if proof_found:
                    generation_result = GenerationResult.FINAL
                else:# elif not proof_found and attempt_idx % 2 == 1:
                    logger.info("Proof generation failed, will retry complete regeneration.")
                    generation_result = GenerationResult.REGENERATE
                # else:
                #     logger.info("Proof generation failed, will retry with the same proof plan.")
    ...
    def _generate_proof_plan(self, problem: LeanProblemView, logger: logging.Logger = None):
        ...
        raw_proof_plan, lemmas, lemma_plans, correctness_plan = proof_planner.solve_intermediate(
            problem_statement=problem.problem_spec_nl,
            problem_spec=problem.problem_spec_formal_ground_truth,
            full_implementation_or_generated_spec=problem.implementation_signature + "\n" + problem.implementation,
            correctness_or_isomorphism_definition=problem.correctness_theorem,
            is_impl_proof_planner=True
        )
        proof_planner.reset()
        lemma_plan_objs = []
    ...
    def _generate_few_shot_proof(
        ...
        few_shot_prover = FewShotImplProverTool(
