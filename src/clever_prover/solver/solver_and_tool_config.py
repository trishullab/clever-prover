...
# from clever_prover.solver.tools.old_code_tool import OldCodeTool # TODO: delete these and large parts of this file
...
from clever_prover.prompters.implementation_planner_prompter import ImplementationPlannerPrompter
from clever_prover.prompters.implementer_prompter import ImplementerPrompter
from clever_prover.prompters.proof_planner_prompter import ProofPlannerPrompter
...
class PrompterType(Enum):
    ...
    ImplementationPlannerPrompter = "ImplementationPlannerPrompter"
    ImplementerPrompter = "ImplementerPrompter"
    ProofPlannerPrompter = "ProofPlannerPrompter"
...
class SolverOrToolType(Enum):
    ...
    ImplementationPlannerTool = "ImplementationPlannerTool"
    ImplementerTool = "ImplementerTool"
    ProofPlannerTool = "ProofPlannerTool"
...
class PrompterConfig:
    ...
    def get_prompter(self) -> typing.Union[Prompter, typing.Tuple[Prompter, Prompter]]:
        ...
        elif self.prompter_type == PrompterType.ImplementationPlannerPrompter:
            return ImplementationPlannerPrompter(system_prompt_path=self.system_prompt_path, example_prompt_path=self.example_prompt_path)
        elif self.prompter_type == PrompterType.ImplementerPrompter:
            return ImplementerPrompter(system_prompt_path=self.system_prompt_path, example_prompt_path=self.example_prompt_path)
        elif self.prompter_type == PrompterType.ProofPlannerPrompter:
            return ProofPlannerPrompter(system_prompt_path=self.system_prompt_path, example_prompt_path=self.example_prompt_path)
        else:
...
class SolverOrToolConfig:
    ...
    def get_solver_or_tool(self, logger: logging.Logger) -> typing.Union[Solver, Tool]:
        ...
        elif self.solver_or_tool_type == SolverOrToolType.ImplementationPlannerTool:
            if self.model_settings.name_or_path not in GLOBAL_MODEL_CACHE:
                model = GptModel(self.model_settings.name_or_path, logger)
                GLOBAL_MODEL_CACHE[self.model_settings.name_or_path] = model
            else:
                model = GLOBAL_MODEL_CACHE[self.model_settings.name_or_path]
            prompter = self.prompter_config.get_prompter()
            return ImplementationPlannerTool(model, prompter, logger, **self.inference_settings.to_dict())
        elif self.solver_or_tool_type == SolverOrToolType.ImplementerTool:
            if self.model_settings.name_or_path not in GLOBAL_MODEL_CACHE:
                model = GptModel(self.model_settings.name_or_path, logger)
                GLOBAL_MODEL_CACHE[self.model_settings.name_or_path] = model
            else:
                model = GLOBAL_MODEL_CACHE[self.model_settings.name_or_path]
            prompter = self.prompter_config.get_prompter()
            return ImplementerTool(model, prompter, logger, **self.inference_settings.to_dict())
        elif self.solver_or_tool_type == SolverOrToolType.ProofPlannerTool:
            if self.model_settings.name_or_path not in GLOBAL_MODEL_CACHE:
                model = GptModel(self.model_settings.name_or_path, logger)
                GLOBAL_MODEL_CACHE[self.model_settings.name_or_path] = model
            else:
                model = GLOBAL_MODEL_CACHE[self.model_settings.name_or_path]
            prompter = self.prompter_config.get_prompter()
            return ProofPlannerTool(model, prompter, logger, **self.inference_settings.to_dict())
        else:
